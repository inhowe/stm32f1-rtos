...
NVIC_PENDSVSET  EQU     0x10000000				; 写NVIC_PENDSVSET到NVIC_INT_CTRL即可触发PendSV中断Value to trigger PendSV exception.
  ...
  ;https://blog.csdn.net/dddxxxx/article/details/47129883 
  AREA |.text|, CODE, READONLY, ALIGN=2 ; 指定该段名称为|.text|, 为只读代码段, 4字节对齐。ALIGN是2的指数
  THUMB	;使用THUMB指令集
...
PendSV_Handler
   ...
   ;保存上文
    SUBS    R0, R0, #0x20                       ; R0减0x20=32往低地址偏移8个寄存器*4字节长度，以保存剩余的8个寄存器。Save remaining regs r4-11 on process stack
